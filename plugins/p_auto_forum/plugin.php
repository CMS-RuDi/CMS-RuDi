...
class p_auto_forum extends cmsPlugin
{
    ...
    {
        switch ( $event ) {
            case 'DELETE_ARTICLE': $this->deleteForum($article);
                break;
            case 'GET_ARTICLE': $article = $this->getForumLink($article);
                break;
            case 'ADD_ARTICLE_DONE': $this->createForum($article);
                break;
            case 'UPDATE_ARTICLE': $this->updateLastForumPost($article);
                break;
        }
    ...
    private function updateLastForumPost($article)
    {
        ...
        $post = $this->inDB->get_fields('cms_forum_threads t, cms_forum_posts p', "t.id = p.thread_id AND t.rel_to='content' AND t.rel_id= '{$article['id']}'", 'p.id', 'p.pubdate ASC');
    ...
    {
        if ( !$this->config['AF_DELETE_THREAD'] ) {
            return;
        }
        ...
        $thread = $this->inDB->get_fields('cms_forum_threads t
                                           INNER JOIN cms_forums f ON f.id = t.forum_id', "rel_to='content' AND rel_id= '{$article_id}'", 't.*, f.NSLeft, f.NSRight');
    ...
    private function checkCatForAdd($cat_id)
    {
        ...
        if ( !$this->config['AF_NOCREATETREAD'] ) {
            return true;
        }
