...
class bbcode
{
            ...
            switch ( $type_of_char ) {
                case 6:
                    if ( 6 == $previous_type ) {
                        ...
                        $tokens[++$token_key] = array( 6, mb_substr($code, $i, 1) );
                    }
                    break;
                case 7:
                    if ( 7 == $previous_type ) {
                        ...
                        $tokens[++$token_key] = array( 7, mb_substr($code, $i, 1) );
                    }
                    break;
                default:
    ...
    public static function autoLink($text)
    {
        ...
        return preg_replace_callback(
                '#<a href="/go/url=([^"]+)"#', create_function(
                        '$matches', 'if (!strstr($matches[1], $_SERVER[\'HTTP_HOST\'])){ return "<a rel=\"nofollow\" title=\"".htmlspecialchars($matches[1])."\" href=\"/go/url=-".base64_encode($matches[1])."\""; } else { return "<a href=\"".htmlspecialchars($matches[1])."\""; }'
                ), $text
        );
    }
    ...
    function font_2html($elem)
    {
        ...
        if ( $color ) {
            $attr .= ' color="' . $this->cleanAttrValue($color) . '"';
        }

        $size = isset($elem['attrib']['size']) ? $elem['attrib']['size'] : '';

        if ( $size ) {
            $attr .= ' size="' . $this->cleanAttrValue($size) . '"';
        }
    ...
    function h1_2html($elem)
    {
        ...
        if ( $align ) {
            $attr .= ' align="' . $this->cleanAttrValue($align) . '"';
        }
    ...
    function quote_2html($elem)
    {
        ...
        $author = $author ? '<strong>' . $author . ':</strong><br>' : '';
    ...
    function url_2html($elem)
    {
        ...
        if ( $name ) {
            $attr .= ' name="' . $this->cleanAttrValue($name) . '"';
        }

        $target = isset($elem['attrib']['target']) ? $elem['attrib']['target'] : '';

        if ( $target ) {
            $attr .= ' target="' . $this->cleanAttrValue($target) . '"';
        }
