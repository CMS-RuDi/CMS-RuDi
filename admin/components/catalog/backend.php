...
if ( $opt == 'hide_item' ) {
    if ( !isset($_REQUEST['item']) ) {
        if ( isset($_REQUEST['item_id']) ) {
            dbHide('cms_uc_items', cmsCore::request('item_id', 'int', 0));
        }
        echo '1';
        exit;
    }
...
if ( $opt == 'submit_cat' || $opt == 'update_cat' ) {
    ...
    if ( cmsCore::request('copy_parent_struct') ) {
        ...
        foreach ( $fstruct as $key => $value ) {
            ...
            if ( $_REQUEST['fformat'][$key] == 'html' ) {
                $fstruct[$key] .= '/~h~/';
            }
            if ( $_REQUEST['fformat'][$key] == 'link' ) {
                $fstruct[$key] .= '/~l~/';
            }
    ...
    if ( $opt == 'submit_cat' ) {
        $cat_id = $inDB->addNsCategory('cms_uc_cats', \cms\plugins::callEvent('catalog.add_category', $cat));
    }
    else {
                        ...
                        <?php for ( $f = 0; $f < 15; $f++ ) { ?>
                            ...
                            if ( @$fstruct[$f] ) {
                                ...
                                if ( mb_strstr($fstruct[$f], '/~m~/') ) {
                                    $makelink    = true;
                                    $fstruct[$f] = str_replace('/~m~/', '', $fstruct[$f]);
                                }
                                else {
                                    $makelink = false;
                                }
