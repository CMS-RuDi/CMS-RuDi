...
class cmsActions
{
    ...
    private static $defaultLogArray = array(
        'pubdate'         => '',
        'user_id'         => '',
        'object'          => '',
        'object_url'      => '',
    ...
    {
        if ( self::getAction($action['name'], false) ) {
            return true;
        }
    ...
    public static function getAction($action_name, $only_tracked = true)
    {
        ...
        return is_array($action) ? \cms\plugins::callEvent('actions.get_action', $action) : false;
    }
    ...
    public static function log($action_name, $params)
    {
        ...
        if ( !$inUser->id && $action_name != 'add_user' ) {
            return false;
        }
        ...
        cmsDatabase::getInstance()->insert('cms_actions_log', \cms\plugins::callEvent('actions.log_action', $params));
    ...
    public static function removeObjectLog($action_name, $object_id, $user_id = false)
    {
        ...
        \cms\plugins::callEvent('actions.delete_object_log', $arg);
    ...
    public static function removeTargetLog($action_name, $target_id, $user_id = false)
    {
        ...
        \cms\plugins::callEvent('actions.delete_target_log', $arg);
    ...
    {
        \cms\plugins::callEvent('actions.delete_log', $id);
    ...
    public static function updateLog($action_name, $params, $object_id = 0, $target_id = 0)
    {
        ...
        \cms\plugins::callEvent('actions.update_log', $arg);
    ...
    public function getActionsLog()
    {
        ...
        $pactions = \cms\plugins::callEvent('actions.get_before_actions', false);
        if ( $pactions !== false ) {
            return $pactions;
        }
        ...
        while ( $action = $this->inDB->fetch_assoc($result) ) {
            ...
            if ( $item_date != $last_date ) {
                switch ( $item_date ) {
                    case $today_date: $date = icms_ucfirst($_LANG['TODAY']);
                        break;
                    case $yesterday_date: $date = icms_ucfirst($_LANG['YESTERDAY']);
                        break;
        ...
        return \cms\plugins::callEvent('actions.get_actions', $actions);
    }
