...
class controller
{
    ...
    {
        return $this->isComponentEnabled($this->name);
    }

    public static function isComponentEnabled($name)
    {
        self::loadComponents();

        return !empty(self::$components[$name]['is_enabled']);
    }
    ...
    public static function loadOptions($component_name)
    {
        ...
        if ( empty(self::$components[$component_name]['options']) ) {
            ...
            $default_config = model::getDefaultConfig($component_name);
...
class cmsController
{
     ...
     */
    public function setCallback($name, $callbacks)
    {
    ...
    public function before($action_name)
    {
        ...
        if ( $this->useSeoOptions && $action_name == 'index' ) {
            ...
            if ( !empty($this->options['seo_keys']) ) {
                $this->cms_template->setPageKeywords($this->options['seo_keys']);
            }
            if ( !empty($this->options['seo_desc']) ) {
                $this->cms_template->setPageDescription($this->options['seo_desc']);
            }
        }
        ...
        if ( is_readable($action_file) ) {
            ...
            if ( method_exists($this, $method_name) ) {
                ...
                if ( $this->name != 'admin' ) {
                    ...
                    if ( $max_params < count($this->current_params) ) {
                        cmsCore::error404();
                    }
                }
