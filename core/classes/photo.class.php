...
class cmsPhoto
{
    ...
    public function wherePeriodIs($period)
    {
        ...
        switch ( $period ) {
            case 'today': $this->inDB->where("DATE_FORMAT(f.pubdate, '%d-%m-%Y')='" . $today . "'");
                break;
            case 'week': $this->inDB->where("DATEDIFF(NOW(), f.pubdate) <= 7");
                break;
            case 'month': $this->inDB->where("DATE_SUB(NOW(), INTERVAL 1 MONTH) < f.pubdate");
                break;
            default: return;
        }
    ...
    {
        $photo_id = $this->inDB->insert('cms_photo_files', \cms\plugin::callEvent('photos.add_photo', $photo));
    ...
    {
        $this->inDB->update('cms_photo_files', \cms\plugin::callEvent('photos.update_photo', $photo), $id);
    ...
    {
        $photo = \cms\plugin::callEvent('photos.delete_photo', $photo);
    ...
    {
        \cms\plugin::callEvent('photos.publish_photo', $photo_id);
    ...
    {
        if ( !($file && is_object($inUploadPhoto)) ) {
            return false;
        }
    ...
    public function getAlbums($parent_id = 0, $differ = '', $recurse = false)
    {
        ...
        return \cms\plugin::callEvent('photos.get_albums', $albums);
    }
    ...
    public function deleteAlbum($album_id, $differ = '', $inUploadPhoto)
    {
        ...
        \cms\plugin::callEvent('photos.delete_album', $album_id);
    ...
    public function getPhotos($show_all = false, $is_comments_count = false)
    {
        ...
        return \cms\plugin::callEvent('photos.get_photos', $photos);
    }
