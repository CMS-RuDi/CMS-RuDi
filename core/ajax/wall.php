...
if (!$inCore->isComponentEnable($component)) {
    cmsCore::halt();
}
...
if (!method_exists($model, 'addWall')) {
    cmsCore::halt();
}
...
if ($do == 'add') {
    if (!$inUser->id) {
        cmsCore::jsonOutput(array(
            'error' => true,
            'text' => $_LANG['ONLY_REG_USER_CAN_WALL']
        ));
    }
    ...
    if (!cmsCore::inRequest('submit')) {
        cmsCore::jsonOutput(array(
            'error' => false,
            'html' => cmsPage::initTemplate('components', 'com_users_addwall', array(
                         'target_id' => $target_id,
                         'component' => $component,
                         'bb_toolbar' => cmsPage::getBBCodeToolbar('message', true, $component, 'wall'),
