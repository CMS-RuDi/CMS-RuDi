...
<div class="bd_links">
    {if $user_id}
        {if $item.user && !$item.user_is_deleted && $item.user_id != $user_id}
            ...
            <span class="bd_message">
                <a class="ajaxlink" title="{$LANG.WRITE_MESS_TO_AVTOR}" href="javascript:void(0)" onclick="users.sendMess('{$item.user_id}', 0, this);return false;">{$LANG.WRITE_MESS_TO_AVTOR}</a>
            </span>
        {/if}
    {/if}

    {if $item.user_login}
        <span class="bd_author">
            <a href="/board/by_user_{$item.user_login}">{$LANG.ALL_AVTOR_ADVS}</a>
        </span>
    {/if}
</div>
