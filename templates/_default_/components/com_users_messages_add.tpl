    ...
    <div style="margin-top:6px; display:block">
        ...
        {if $id_admin && !$is_reply_user}
            {if !$id}
                ...
                <a href="javascript:" class="s_usr ajaxlink" onclick="$('.s_grp').fadeIn();
                $('.s_usr').hide();
                $('#send_to_group').val(1);">
                    {$LANG.SEND_TO_GROUP}
                </a>
                <a href="javascript:" class="s_grp ajaxlink" onclick="$('.s_grp').hide();
                $('.s_usr').fadeIn();
                $('#send_to_group').val(0);" style="display:none">
                    {$LANG.SEND_TO_FRIEND}
                </a>
            {/if}
            <label>
