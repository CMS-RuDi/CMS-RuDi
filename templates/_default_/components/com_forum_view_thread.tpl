    ...
    {foreach key=pid item=post from=$posts}
        <tr>
            <td colspan="2" class="darkBlue-LightBlue">
                <div class="post_date">{if $post.pinned && $num > 1}<img src="/templates/{template}/images/icons/forum/sticky.png" width="14px;" alt="{$LANG.ATTACHED_MESSAGE}" title="{$LANG.ATTACHED_MESSAGE}" />  {/if}<strong><a name="{$post.id}" href="/forum/thread{$thread.id}-{$page}.html#{$post.id}">#{$num}</a></strong> - {$post.fpubdate}, {$post.wday}</div>
                    ...
                    <div class="msg_links">
                        <a href="javascript:" onclick="forum.addQuoteText(this);
                            return false;" rel="{$post.nickname|escape:html}" class="ajaxlink" title="{$LANG.ADD_SELECTED_QUOTE}">{$LANG.ADD_QUOTE_TEXT}</a> | <a href="/forum/thread{$thread.id}-quote{$post.id}.html" title="{$LANG.REPLY_FULL_QUOTE}">{$LANG.REPLY}</a>
                        {if $is_admin || $is_moder || $post.is_author_can_edit}
                            | <a href="/forum/editpost{$post.id}-{$page}.html">{$LANG.EDIT}</a>
                            {if $num > 1}
                                {if $is_admin || $is_moder}
                                    | <a href="javascript:" onclick="forum.movePost('{$thread.id}', '{$post.id}');
                                        return false;" class="ajaxlink" title="{$LANG.MOVE_POST}">{$LANG.MOVE}</a>
                                    {if !$post.pinned}
        ...
        <tr class="posts_table_tr">
            <td class="post_usercell" width="140" align="center" valign="top" height="150">
                <div>
                    <a class="post_userlink" href="javascript:" onclick="addNickname(this);
                        return false;" title="{$LANG.ADD_NICKNAME}" rel="{$post.nickname|escape:html}" >{$post.nickname|escape:html}</a>
                </div>
        ...
        {if $user_id && $is_can_add_post}
            ...
            <div class="forum_fast_form">
                <form action="/forum/reply{$thread.id}.html" method="post" id="msgform">
                    ...
                    <div class="forum_fast_submit" style="float:right;padding:5px;"><input type="button" value="{$LANG.SEND}" onclick="$(this).prop('disabled', true);
                        $('#msgform').submit();" /></div>
