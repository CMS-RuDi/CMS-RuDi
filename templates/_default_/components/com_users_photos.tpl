{if ($my_profile || $is_admin) && $album_type == 'private'}
    <div class="float_bar">
        ...
        <a href="/users/{$user_id}/delalbum{$album.id}.html" onclick="if (!confirm('{$LANG.DELETE_ALBUM_CONFIRM}')) {
                    return false;
                }" class="usr_del_album"><span class="ajaxlink">{$LANG.DELETE_ALBUM}</span></a>
    </div>
{/if}
