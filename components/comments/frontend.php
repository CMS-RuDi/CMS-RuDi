...
function comments($target = '', $target_id = 0, $labels = array(), $can_delete = null)
{
    ...
    if ( !in_array($do, array( 'add', 'edit', 'delete' )) && $target && $target_id ) {
        if ( !$model->config['cmm_ajax'] ) {
            ...
            $comments = \cms\plugins::callEvent('comments.before_show', $model->getComments(!($inUser->is_admin || $model->is_can_moderate), true));
    ...
    if ( $do == 'add' ) {
        ...
        if ( $comment['published'] ) {
            ...
            cmsUser::sendUpdateNotify($comment['target'], $comment['target_id'], array( 'link'        => $comment['target_link'] . '#c' . $comment_id,
        ...
        if ( $model->config['email'] ) {
            $mailmsg = str_replace(array( '{sitename}', '{date}', '{from}',
                '{subjtitle}', '{targetlink}', '{content}' ), array( $inConf->sitename, date('d/m/Y (H:i)'), $from_nick,
                stripslashes($comment['target_title']), $targetlink, strip_tags($comment['content']) ), cmsCore::getLanguageTextFile('newcomment_admin'));
    ...
    if ( $do == 'edit' ) {
        ...
        if ( !$model->is_can_moderate &&
                !$inUser->is_admin &&
                !(($inUser->id == $comment['user_id']) && $comment['is_editable']) ) {
            cmsCore::error404();
        }
        ...
        $com_new = \cms\plugins::callEvent('comments.get_item', $com_new);
