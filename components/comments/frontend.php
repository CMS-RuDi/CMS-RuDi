...
function comments($target = '', $target_id = 0, $labels = array(), $can_delete = null)
{
    ...
    if ( !in_array($do, array( 'add', 'edit', 'delete' )) && $target && $target_id ) {
        if ( !$model->config['cmm_ajax'] ) {
            ...
            $comments = \cms\events::call('comments.before_show', $model->getComments(!($inUser->is_admin || $model->is_can_moderate), true));
    ...
    if ( $do == 'edit' ) {
        ...
        $com_new = \cms\events::call('comments.get_item', $com_new);
