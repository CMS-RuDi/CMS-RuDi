...
//======================== Просмотр темы форума  =============================//
    ...
    if ( $do == 'thread' ) {
        ...
        cmsPage::initTemplate('components', 'com_forum_view_thread')->
                ...
                assign('prev_thread', $inDB->get_fields('cms_forum_threads', "id < '{$thread['id']}' AND forum_id = '{$thread['forum_id']}'", 'id, title', 'id DESC'))->
                assign('next_thread', $inDB->get_fields('cms_forum_threads', "id > '{$thread['id']}' AND forum_id = '{$thread['forum_id']}'", 'id, title', 'id ASC'))->
                assign('posts', $posts)->
    ...
    if ( in_array($do, array( 'newthread', 'newpost', 'editpost' )) ) {
        ...
        if ( in_array($do, array( 'newthread', 'newpost' )) ) {
            if ( ($inUser->karma < $model->config['min_karma_add']) && !$inUser->is_admin ) {
                cmsCore::addSessionMessage(sprintf(
                                $_LANG['ADD_KARMA_LIMIT'], cmsCore::spellCount($model->config['min_karma_add'], $_LANG['KPOINT1'], $_LANG['KPOINT2'], $_LANG['KPOINT10']), $inUser->karma
                        ), 'error');
        ...
        if ( !cmsCore::inRequest('gosend') ) {
            ...
            if ( in_array($do, array( 'newthread', 'newpost' )) ) {
                ...
                if ( $do == 'newpost' ) {
                    cmsUser::sendUpdateNotify('forum', $thread['id'], array( 'link'        => '/forum/thread' . $thread['id'] . '-' . $total_pages . '.html#' . $post_id,
...
//========================== Операции с темами ===============================//
    ...
    if ( in_array($do, array( 'movethread', 'renamethread', 'deletethread', 'close', 'pin', 'pin_post', 'move_post' )) ) {
        ...
        if ( $do == 'move_post' ) {
            ...
            \cms\plugins::callEvent('forum.move_post', array( 'thread' => $thread, 'post_id' => $post_id ));
