...
function shopIsInCart($item_id = 0)
{
    ...
    if ( $user_id ) {
        $user_sql = "(user_id='$user_id' OR session_id='$sid')";
    }
    else {
        $user_sql = "(user_id=0 AND session_id='$sid')";
    }
...
function shopDiscountPrice($item_id, $cat_id, $price)
{
    ...
    if ( $inDB->num_rows($res) ) {
        while ( $dis = $inDB->fetch_assoc($res) ) {
            if ( $dis['unit'] == '%' ) {
                $price = $price + ($price * ($dis['value'] / 100)) * $dis['sign'];
            }
            if ( $dis['unit'] == $_LANG['CURRENCY'] ) {
                $price = $price + $dis['value'] * $dis['sign'];
            }
        }
    }
    return $price;
...
function shopItemsCounter($cartitem_id, $count, $canmany = 1)
{
    ...
    if ( $canmany ) {
        $count_field = '<input type="text" style="text-align: center; font-size:9px; border:solid 1px black" size="5" name="kolvo[' . $cartitem_id . ']" value="' . $count . '"/>';
    }
    else {
        $count_field = '<input type="hidden" name="kolvo[' . $cartitem_id . ']" value="1"/>1';
    }
