...
function orderForm($orderby, $orderto, $shop = false)
{
    ...
    if ( $shop ) {
        ...
        $html .= '>' . $_LANG['ORDERBY_PRICE'] . '</option>' . "\n";
    }
    ...
    $html .= '>' . $_LANG['ORDERBY_TITLE'] . '</option>' . "\n";
    $html .= '<option value="pubdate" ';

    if ( $orderby == 'pubdate' ) {
        $html .= 'selected';
    }

    $html .= '>' . $_LANG['ORDERBY_DATE'] . '</option>' . "\n";
    ...
    $html .= '>' . $_LANG['ORDERBY_RATING'] . '</option>' . "\n";
    $html .= '<option value="hits" ';

    if ( $orderby == 'hits' ) {
        $html .= 'selected';
    }

    $html .= '>' . $_LANG['ORDERBY_HITS'] . '</option>' . "\n";
    ...
    $html .= '>' . $_LANG['ORDERBY_DESC'] . '</option>' . "\n";
...
function subCatsList($parent_id = 0, $left_key = 0, $right_key = 0)
{
    ...
    if ( !$parent_id ) {
        $parent_id = $inDB->get_field('cms_uc_cats', 'parent_id=0', 'id');
    }
...
//====================== Поиск по первой букве в названии ====================//
...
//============================ Просмотр категории ============================//
    ...
    if ( $do == 'cat' ) {
        ...
        if ( !isset($findsql) ) {
            ...
            $sql = $findsql;
            if ( !$advsearch ) {
                $inPage->addPathway(icms_ucfirst($query));
            }
            else {
                ...
                foreach ( $fstruct as $key => $value ) {
                    if ( $fields_show < $cat['fields_show'] ) {
                        if ( $item['fdata'][$key] ) {
                            ...
                            if ( isset($query) ) {
                                if ( mb_stristr($field, $query) ) {
                                    $field .= '<span class="uc_findsame"> &larr; <i>' . $_LANG['MATCHE'] . '</i></span>';
                                }
                            }
                            $fields_show++;
...
//============================ Просмотр материала ============================//
    ...
    if ( $do == 'item' ) {
        ...
        if ( $item['meta_keys'] ) {
            $inPage->setKeywords($item['meta_keys']);
        }

        if ( $item['meta_desc'] ) {
            $inPage->setDescription($item['meta_desc']);
        }
    ...
    if ( $do == 'submit_item' ) {
        ...
        if ( $opt == 'add' ) {
            ...
            if ( !$cfg['premod'] || $inUser->is_admin ) {
                \cms\plugin::callEvent('catalog.add_item_done', $item);
...
//=========================== Удаление материала =============================//
    ...
    if ( $do == 'delete_item' ) {
        ...
        if ( !($item['user_id'] == $inUser->id || $inUser->is_admin) ) {
            cmsCore::error404();
        }
