...
class cms_model_search
{
    ...
    protected static function getComponentsArrayForSearch()
    {
        ...
        if ( !$from_component || !is_array($from_component) ) {
            return array();
        }
    ...
    public function wherePeriodIs()
    {
        ...
        switch ( $this->from_pubdate ) {
            ...
            case 'w': $this->inDB->where("DATEDIFF(NOW(), pubdate) <= 7");
                break;
            case 'm': $this->inDB->where("DATE_SUB(NOW(), INTERVAL 1 MONTH) < pubdate");
                break;
            case 'y': $this->inDB->where("DATE_SUB(NOW(), INTERVAL 1 YEAR) < pubdate");
                break;
            default: return;
        }
        return;
    ...
    public function getResults()
    {
        ...
        return \cms\plugin::callEvent('search.get_result', $rs);
    }
    ...
    protected function checkArrayResults($input_array = array())
    {
        ...
        foreach ( $input_array as $k => $v ) {
            if ( !isset($this->default_array_result[$k]) ) {
                unset($input_array[$k]);
            }
        }
    ...
    {
        if ( !cmsUser::sessionGet('searchquery') ) {
            return $text;
        }
