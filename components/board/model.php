...
class cms_model_board
{
    ...
    public function getCategory($category_id = 0)
    {
        ...
        if ( !$category['obtypes'] ) {
            ...
            if ( !$category['obtypes'] ) {
                $category['obtypes'] = $this->config['obtypes'];
            }
        }
    ...
    public function getBoardCities($selected = '', $cat = array())
    {
        ...
        if ( empty($cat['cat_city']) ) {
            $cat['cat_city'] = $this->getCatCity();
        }
    ...
    public function checkAdd($cat)
    {
        ...
        if ( cmsUser::getInstance()->is_admin ) {
            return true;
        }

        // настройки группы всегда приоритетней
        if ( !$this->is_can_add_by_group ) {
            return false;
        }
